...
base_download_url = "https://mrgdatashare.robots.ox.ac.uk/download/?filename=datasets/"
...
class Scraper:
    ...
    def scrape(self, url_handler):
        ...
        local_file = url_handler.local_file_path
        initial_pos = 0
        file_mode = 'wb'
        
        # 将请求头统一放这里发送（原代码里的修改 response header 是无效的）
        headers = {
            'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36'
        }

        # 🌟 断点续传逻辑：检查文件是否已存在，如果存在则获取文件大小并请求后续部分
        ...
        # 🌟 核心修复：加上 timeout=60。如果 60 秒没有数据传输，自动抛出异常而不是死锁
        result = self.session_requests.get(url_handler.file_url, stream=True, headers=headers, timeout=60)

        # 处理服务器返回 HTML（登录过期）的情况
        while 'html' in result.headers.get('content-type', 'html'):
            print(f"Got html file as result. Wait {self.relogin_duration} seconds and re-loging...")
            time.sleep(self.relogin_duration)
            self.login()
            result = self.session_requests.get(url_handler.file_url, stream=True, headers=headers, timeout=60)

        # 检查服务器是否支持断点续传 (206 Partial Content)
        if result.status_code == 200 and initial_pos > 0:
            print("服务器不支持断点续传，将从头开始下载...")
            initial_pos = 0
            file_mode = 'wb'
        elif result.status_code not in (200, 206):
            raise ValueError(f"bad file_url (HTTP {result.status_code}): " + url_handler.file_url)

        print(f"downloading local_file_path: {local_file} (Mode: {file_mode})")
        ...
        with open(local_file, file_mode) as file_handle:
            # 🌟 进度条加入 initial 参数，完美适配断点续传的显示
            with tqdm(total=total_size, initial=initial_pos, unit='B', unit_scale=True, unit_divisor=1024) as pbar:
                for chunk in result.iter_content(chunk_size=throttle.chunk_length):
...
if __name__ == "__main__":
    ...
    argument_parser.add_argument(
        "--username", dest="username", help="Registered username for " + login_url)
    argument_parser.add_argument(
        "--password", dest="password", help="Registered password for " + login_url)
    argument_parser.add_argument(
        "--datasets_file", dest="datasets_file", help="List of dataset names from " + datasets_url + " - separated by new lines.")
    argument_parser.add_argument(
        "--downloads_dir", dest="downloads_dir", default=downloads_dir_example, help="Root download directory e.g. " + downloads_dir_example)
    argument_parser.add_argument(
        "--period_duration", dest="period_duration", type=int, default=default_period_duration)
    argument_parser.add_argument(
        "--chunk_length", dest="chunk_length", type=int, default=default_chunk_length)
    argument_parser.add_argument(
        "--chunks_per_period", dest="chunks_per_period", type=int, default=default_chunks_per_period)
    argument_parser.add_argument(
        "--relogin_duration", dest="relogin_duration", type=int, default=default_relogin_duration)
    argument_parser.add_argument(
        "--reconnection_duration", dest="reconnection_duration", type=int, default=default_reconnection_duration)
    argument_parser.add_argument(
        "--nb_tries", dest="nb_tries_reconnection", type=int, default=default_nb_tries_reconnection)
    argument_parser.add_argument(
        "--choice_sensors", dest="choice_sensors", type=str, default=default_choice_sensors)
    argument_parser.add_argument(
        "--choice_runs_file", dest="choice_runs_file", type=str, default=default_choice_runs_file)
    
    # 🌟 新增参数：指定存放 .done 文件的独立目录
    argument_parser.add_argument(
        "--done_dir", dest="done_dir", type=str, default="./done_markers", 
        help="Directory to store .done files for each downloaded sensor")
    ...
    for dataset in datasets:
        ...
        # iterate file patterns (sensors)
        for file_pattern in dataset["file_patterns"]:
            
            # 🌟 核心逻辑：检查该文件是否已经下载并解压过
            done_marker_name = f"{dataset['dataset']}_{file_pattern}.done"
            done_marker_path = os.path.join(args.done_dir, done_marker_name)
            
            if os.path.exists(done_marker_path):
                print(f"✅ 文件已被处理过，跳过: {done_marker_name}")
                continue # 直接跳过当前文件，进行下一个文件的下载
            ...
            for i in range(args.nb_tries_reconnection):
                try:
                    ...
                    if file_was_found:
                        break
                # 🌟 核心修复：捕获更多的网络异常类型，确保超时后能够触发等待和重试
                except (requests.exceptions.ChunkedEncodingError, requests.exceptions.ConnectionError, requests.exceptions.ReadTimeout) as e:
                    print(f"\n[网络异常] {type(e).__name__} on try {i+1}/{args.nb_tries_reconnection}.")
                    print(f"等待 {args.reconnection_duration} 秒后重新连接并续传...")
            ...
            if file_was_found:
                ...
                # 🌟 核心逻辑：解压成功后，生成针对此单一文件的 .done 标记
                with open(done_marker_path, 'w') as f:
                    f.write("Done at " + str(datetime.datetime.now()))
                print(f"🏁 成功打上文件级标记: {done_marker_path}")
